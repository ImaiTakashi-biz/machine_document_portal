...
- 明確にプリンターへ送信できなかった場合は `PRINT_RETRY_DELAYS_SECONDS` の間隔で自動再実行する。現在の設定は3分後、5分後、10分後（`180,300,600`）
...
本番へ切り替える際は、従来の印刷スクリプトやタスクスケジューラを先に停止し、Google Sheets、SharePoint、ARAICHAT、NAS、プリンターを確認したうえで、13:00より前に `SCHEDULED_OPERATIONS_ENABLED=true` へ変更してNSSMサービスを再起動します。15:00以降に有効化すると、その日の13:00処理と15:00処理を直ちに開始する可能性があります。従来処理と同時に動かすと二重通知・二重印刷になるため、並行稼働は行いません。
